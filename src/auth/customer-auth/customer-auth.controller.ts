...
export class CustomerAuthController {
  ...
  @ApiResponse({ status: 400, description: 'Validation failed or customer already exists.' })
  @Post('/register')
  ...
  @ApiResponse({ status: 302, description: 'Redirects to customer dashboard after successful authentication.' })
  @UseGuards(CustomerGoogleAuthGuard)
  @Get('/google/callback')
  ...
  @ApiCookieAuth('customer_access_token')
  @ApiResponse({ status: 200, description: 'Returns current customer profile.' })
  @ApiResponse({ status: 401, description: 'Unauthorized.' })
  @UseGuards(CustomerJwtAuthGuard)
  @Get('/profile')
