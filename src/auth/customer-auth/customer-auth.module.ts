  ...
  providers: [
    CustomerAuthService,
    {
      ...
      useFactory: (authService: CustomerAuthService) => {
        const Strategy = createLocalStrategy('customer-local', authService);
        return new Strategy();
      },
    ...
    },
    {
      ...
      useFactory: (configService: ConfigService, authService: CustomerAuthService) => {
        const Strategy = createGoogleStrategy(
          {
            name: 'customer-google',
            callbackURL: configService.get('GOOGLE_CUSTOMER_CALLBACK_URL') ?? 
                         configService.get('BACKEND_URL') + '/auth/customer/google/callback',
          },
          authService,
        );
        return new Strategy(configService);
