...
@Module({
  ...
  providers: [
    ...
    },
    {
      ...
      useFactory: (configService: ConfigService, authService: HostAuthService) => {
        const Strategy = createGoogleStrategy(
          {
            name: 'host-google',
            callbackURL: configService.get('GOOGLE_HOST_CALLBACK_URL') ?? 
                         configService.get('BACKEND_URL') + '/auth/host/google/callback',
          },
          authService,
        );
        return new Strategy(configService);
